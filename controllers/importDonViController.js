// controllers/importDonViController.js
...
exports.importFromExcel = async (req, res) => {
    try {
        ...
        console.log(`Đã đọc ${data.length} dòng từ file`);
        ...
        for (let i = 1; i < data.length; i++) {
            try {
                ...
                const MaDV = row[0].toString();
                const TenDV = row[1] ? row[1].toString() : '';

                console.log(`Đang xử lý dòng ${i+1}:`, { MaDV, TenDV });
                ...
                if (existing) {
                    ...
                    existing.TenDV = TenDV;
                    await existing.save();
                } else {
                    // Tạo đơn vị mới
                    const newDonVi = new DonVi({
                        MaDV,
                        TenDV
                    });
                    await newDonVi.save();
        ...
        let message = `✅ Đã import thành công ${successCount} đơn vị.`;
        if (errorCount > 0) {
